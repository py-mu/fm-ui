<template>
  <a-layout :class="['admin-layout', 'beauty-scroll']">
    <drawer v-if="isMobile" v-model="drawerOpen">
      <side-menu :theme="theme.mode" :menuData="menuData" :collapsed="false" :collapsible="true"
    ...
    <a-layout class="admin-layout-main beauty-scroll">
      <admin-header :class="[{'fixed-tabs': fixedTabs, 'fixed-header': fixedHeader, 'multi-page': multiPage}]"
                    :style="headerStyle" :menuData="headMenuData" :collapsed="collapsed" :show-setting="showSetting"
                    @toggleCollapse="toggleCollapse" @showSetting="handlerSetting"/>
      <a-layout-header
...
export default {
  name: 'AdminLayout',
  components: {Setting, SideMenu, PageFooter, AdminHeader, Drawer},
  data() {
    return {
  ...
  computed: {
    ...
    sideMenuWidth() {
      return this.collapsed ? '80px' : '210px'
    },
    headerStyle() {
