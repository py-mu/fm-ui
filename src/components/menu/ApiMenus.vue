  ...
  <div class="left-box">
    ...
    <a-menu mode="inline" class="menu-box beauty-scroll" @select="onSelect" :defaultSelectedKeys="defaultSelectedKeys">
      <a-sub-menu :key="router.path + i" v-for="(router, i) in paths">
        ...
        <a-menu-item :key="router.path + '/' + _router.path " v-for="(_router) in router.children"
          ...
          <span :class="[_router.deprecated ? 'deprecated-text': '', 'display-api-name']" :title=" _router.name">
               {{ _router.name }}
            </span>
          <a-badge color="orange" v-if="_router.deprecated"/>
        </a-menu-item>
      </a-sub-menu>
    </a-menu>
  </div>
...
export default {
  name: "ApiMenus",
  props: ['paths', 'defaultSelectedKeys'],
  methods: {
    onSelect(obj) {
