...
function parseRoutes(routesConfig, routerMap) {
    let routes = []
    // console.log("parseRoutes routesConfig: ",routesConfig)
    // console.log("parseRoutes routerMap: ",routerMap)
    routesConfig.forEach(item => {
        // 获取注册在 routerMap 中的 router，初始化 routeCfg
        let router = undefined, routeCfg = {}
        if (typeof item === 'string') {
            router = routerMap[item]
            // console.log("parseRoutes router: ",router)
            // if(router == undefined){
            //   console.log("parseRoutes item: ",item)
            // }
 ...
 * @param menus
 */
function addRouterWhenParseSuccess(menus) {
    const {router, store, i18n} = appOptions
    let routesConfig = router.options.routes
    if (menus) {
        for (const i in routesConfig) {
            if (routesConfig[i].path === '/') {
                for (const j in routesConfig[i].children) {
                    if (routesConfig[i].children[j].path === 'api-list') {
                        routesConfig[i].children[j].children = menus
                        break
                    }
                }
            }
